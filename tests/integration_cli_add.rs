...
fn test_add_thought() {
    ...
    cmd.arg("--database")
        ...
        .stdout(predicate::str::contains(
            "Adding thought: This is a test thought",
        ))
    ...
    let mut stmt = conn
        .prepare("SELECT id, content FROM thoughts WHERE id = ?")
        .unwrap();
    let thought = stmt
        .query_row(params![1], |row| {
            Ok((
...
fn test_add_multiple_thoughts() {
    ...
    let conn = Connection::open(db_path).unwrap();
    let mut stmt = conn
        .prepare("SELECT id, content FROM thoughts ORDER BY id")
        .unwrap();
    let thoughts: Vec<(i64, String)> = stmt
        .query_map([], |row| {
            Ok((
                row.get::<_, i64>(0).unwrap(),
                row.get::<_, String>(1).unwrap(),
...
fn test_add_thought_custom_database() {
    ...
    let conn = Connection::open(&db_path).unwrap();
    let mut stmt = conn
        .prepare("SELECT content FROM thoughts WHERE id = 1")
        .unwrap();
